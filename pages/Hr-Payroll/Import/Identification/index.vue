                ...
                <div ref="action" class="z-30 fixed action-container scaffold-border-bottom">
                    ...
                    <div class="mx-auto max-w-screen-2xl space-y-2 p-[20px]">
                        ...
                        <div class="flex flex-row flex-wrap gap-2 items-center min-h-8">
                            <NuxtLink class="inline-flex" :to="`${baseURL}/api/employee-identification-import-template`">
                ...
                <div class="mx-auto max-w-screen-2xl">
                    <div class="px-[20px] allocate">
                        <DataTable
                            ...
                            <template v-slot:cell.identification_type="{cell, slot, scrollReference}">
                                <div v-if="!cell.isEditing" class="p-[3px] cursor-pointer" @click="editRow(cell)">
                                    {{cell.identification_type}}
                                </div>
                                <div v-else class="mx-0.5 flex items-center">
                                    <Input high-light-all-text-on-focus v-model="cell.identification_type" :size="slot.inputSize" />
                                </div>
                            </template>
                            <template v-slot:cell.number="{cell, slot, scrollReference}">
                                ...
                                <div v-else class="mx-0.5 flex items-center">
                                    <Input high-light-all-text-on-focus v-model="cell.number" :size="slot.inputSize" />
                                </div>
                            </template>
                            <template v-slot:cell.readable_number="{cell, slot, scrollReference}">
                                <div v-if="!cell.isEditing" class="p-[3px] cursor-pointer" @click="editRow(cell)">
                                    {{cell.readable_number}}
                                </div>
                                <div v-else class="mx-0.5 flex items-center">
                                    <Input high-light-all-text-on-focus v-model="cell.readable_number" :size="slot.inputSize" />
                                </div>
                            </template>
...
import type {TableHeaderT, TableRowPayloadT, TableSupHeaderT, TableSupRowT} from "@/public/js/types/data";
import type {EmployeeIdentificationPreImportT, ImportRow} from "@/public/js/types/import";
import {storeToRefs} from "pinia";

useHead({titleTemplate: (titleChunk) => {return `Import Identification`}});
...
const preImportSupHeaders = reactive<TableSupHeaderT[]>([
    ...
    {text: 'Required', colspan: 1, value: 'identification_type', alignHeader: 'left', justifyData: 'top'},
    {text: 'Required', colspan: 1, value: 'number', width: '8rem', maxWidth: '8rem'},
    {text: 'Optional', colspan: 1, value: 'readable_number', alignHeader: 'left', justifyData: 'top'},
    {text: ''},
]);

const countryIdentificationTypes = ref({});

const preImportSupRows = computed<TableSupRowT[]>(() => {

    return [
        {
            'identification_type': "One of the following: \n" + Object.values(countryIdentificationTypes.value).join("\n"),
            'number': "",
            'readable_number': "",
        }
    ];
...
const setCountryIdentificationTypesHeaders = () => {

    let companyCountryIso2 = selectedAssociatedCompany.value?.payload?.country?.iso2;

    if(companyCountryIso2 == 'PH'){
        countryIdentificationTypes.value = PH_IDENTIFICATION_TYPE_HEADERS;
    }
}

//Render identification types on navigate
if(clientReadyState.value){
    onMounted(async () => {
        await nextTick(() => {
            setCountryIdentificationTypesHeaders();
        });
    });
}

//Render identification types on load
...
const preImportHeaders = reactive<TableHeaderT[]>([
    ...
    { text: 'Identification type', value: 'identification_type', alignData: 'left'},
    { text: 'Number', value: 'number', alignData: 'left'},
    { text: 'Readable number', value: 'readable_number', alignData: 'left'},
    { text: 'Validation Result', value: 'validation', alignData: 'left'}
]);

const preImportData = ref<EmployeeIdentificationPreImportT[]>([]);
const preImportDataThatHasValidationError = computed(() => {
    return preImportData.value.filter((item: EmployeeIdentificationPreImportT) => {
...
const selectedPreImportDataThatHasValidationError = computed(() => {
    return selectedPreImportData.value.filter((item: EmployeeIdentificationPreImportT) => {
        return item.validation_errors.length > 0;
    });
})
...
const transformValidated = (validated: EmployeeIdentificationPreImportT[]) => {

    return _map(validated, (item: EmployeeIdentificationPreImportT) => {
...
const read = async () => {
    ...
    if (file) {
        ...
        await laraFetch("/api/employee-identification-import-validate", {
            method: 'POST',
            body: formData,
        },{
...
const reValidate = async () => {
    ...
    await laraFetch("/api/employee-identification-import-re-validate", {
        method: 'POST',
        body: {
        ...
        onSuccessResponse: async (request, options, response) => {
            ...
            _get(response, '_data.values.validated', []).forEach((validated: EmployeeIdentificationPreImportT) => {

                let row:(EmployeeIdentificationPreImportT | undefined) = _find(preImportData.value, {id: validated.id});
...
const editRow = (cell: EmployeeIdentificationPreImportT) => {
    cell.isEditing = true;
};
const save = async () => {
    ...
    await laraFetch("/api/employee-identification-import-save", {
        method: 'POST',
        body: {
            account_id: selectedAssociatedCompanyAccountId.value,
            company_id: selectedAssociatedCompanyId.value,
        ...
        onSuccessResponse: async (request, options, response) => {
            ...
            if(!_isEmpty(saved)){
                ...
                preImportData.value = _filter(preImportData.value, (item: EmployeeIdentificationPreImportT) => {
