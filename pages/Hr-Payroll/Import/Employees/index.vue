                                ...
                                <div class="h-8 flex flex-row items-center scaffold-border px-2">
                                    <label class="flex items-center">
                                        <Checkbox
                                            ...
                                            name="auto-create-user-on-provided-office-email"
                                            v-model="createUserOnProvidedOfficeEmail"
                                            :size="'md'"
                                            :label="'Auto-create user on provided office email'" />
                                    </label>
                                </div>
                            </div>
                        </div>
                ...
                <div class="mx-auto max-w-screen-2xl">
                    <div class="px-[20px] allocate">
                        <DataTable
                            ...
                            <template v-slot:cell.birth_date="{cell, slot, scrollReference}">
                                ...
                                <div v-else class="mx-0.5 flex items-center">
                                    <InputWithIcon
                                        ...
                                        v-model="cell.birth_date"
                                        :override="{font_family_class: 'font-sans'}"
                                        :id="`${cell.id}-birth-date`"
                                        :size="slot.inputSize" />
                                </div>
                            </template>
...
import type {EmployeePreImportT, EmploymentProfilePreImportT, ImportRow} from "@/public/js/types/import";
...
const preImportSupHeaders = computed<TableSupHeaderT[]>(() => {
    return [
        ...
        {text: 'Required', colspan: 1, value: 'birth_date', alignHeader: 'left', justifyData: 'top'},
...
const preImportHeaders = computed<TableHeaderT[]>(() => {
    return [
        ...
        { text: 'Number', value: 'number', alignData: 'left', width: '85px', maxWidth: '85px'},
        { text: 'Family name', value: 'family_name', alignData: 'left', width: '85px', maxWidth: '85px'},
        { text: 'Given name', value: 'given_name', alignData: 'left', width: '85px', maxWidth: '85px'},
        { text: 'Birth Date', value: 'birth_date', alignData: 'left', width: '10rem', maxWidth: '10rem'},
...
const createUserOnProvidedOfficeEmail = ref(false);
...
const save = async () => {
    ...
    await laraFetch("/api/employee-import-save", {
        ...
        body: {
            account_id: selectedAssociatedCompanyAccountId.value,
            company_id: selectedAssociatedCompanyId.value,
            create_user_on_provided_office_email: createUserOnProvidedOfficeEmail.value,
...
const datePickersComputed = computed<DatePickerT[]>(() => {
    ...
    let editRows: ImportRow[] = _filter(preImportData.value, (employee: ImportRow) => {
        return employee.isEditing;
    });

    let birthDates: DatePickerT[] =  _map(editRows, (employee: EmployeePreImportT) => {

        return {
            id: `${employee.id}-birth-date`,
            type: 'date',
            selectedCallback: (payload: {value: string}) => {
                employee.birth_date = payload.value;
            }
        };
    });

    datePickers = datePickers.concat(birthDates);
