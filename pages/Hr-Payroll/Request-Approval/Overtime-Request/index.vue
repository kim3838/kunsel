...
const overtimeRequestsExecute = async() =>{
    ...
    await laraFetch(`/api/overtime-requests`, {
        method: 'GET',
...
const confirmDeleteSelected = () => {
    ...
    if(selectedIds.length == 0){
        ...
        useNuxtApp().$promptStore.setPrompt({
            ...
            message: `No selected overtime request to delete.`,
            action: {
                callback: () => {},
                label: 'Okay'
            }
...
const modalSubmitPath = computed(() => {
    if(!creatingOvertime.value){
        return `/api/overtime-request/${stagedOvertimeRequest.value.id}`;
    } else {
        return `/api/overtime-request`
    }
});
