            ...
            <div class="mx-auto max-w-screen-xl">
                                    ...
                                    <div class="grid gap-2 grid-cols-4">
                                        ...
                                        <div class="col-span-4 md:col-span-3 lg:col-span-2">
                                            ...
                                            <SingleSelectPaginated
                                                ...
                                                :disabled="modalDisableActions || !assignedShiftSelectionsOptions.selected"
                                                drop-shadow
                                                value-persist
                                                :selection-max-viewable-line="10"
                ...
                <div class="px-[20px] space-y-2">
                    <div class="flex flex-row flex-wrap gap-2 items-center min-h-8" :class="[disableActions ? 'pointer-events-none' : '']">
                        ...
                        <Button v-if="leaveRequests.successful" :variant="'outline'" :size="'sm'" :icon="'ph:trash-simple'" :disabled="disableActions" :label="'Bulk delete'" @click="confirmDeleteSelected()" />
...
const leaveRequestsExecute = async() =>{
    ...
    await laraFetch(`/api/employee-portal-leave-requests`, {
        method: 'GET',
...
const modalSubmitPath = computed(() => {
    if(!creatingLeaveRequest.value){
        return `/api/employee-portal-leave-request/${stagedLeaveRequest.value.id}`;
    } else {
        return `/api/employee-portal-leave-request`
    }
});
...
const modalForm = computed(()=>{
    return {
        account_id: selectedAssociatedCompanyAccountId.value,
        company_id: selectedAssociatedCompanyId.value,
        employee_id: userCompanyEmployee.value?.id,
    ...
    await laraFetch(modalSubmitPath.value, {
        ...
        onSuccessResponse: async (request, options, response) => {
            ...
            useNuxtApp().$promptStore.setPrompt({
                ...
                icon: null,
                title: `Request successful`,
                message: 'Leave request submitted, you can monitor your request status at the Home page.',
                action: {
                    callback: () => {},
                    label: 'Okay'
                }
